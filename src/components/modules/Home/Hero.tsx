...
export function Hero() {
  ...
  return (
    <section className="relative bg-background text-foreground">
      <div className="pointer-events-none absolute right-6 top-14 hidden h-52 w-40 overflow-hidden rounded-[28px] border border-border/80 bg-card shadow-[0_18px_40px_-22px_rgba(15,23,42,0.45)] lg:block xl:hidden">
        <Image
          src={heroDoctorImage}
      ...
      <div className="mx-auto w-full max-w-full px-4 pb-12 pt-0 sm:px-6 md:pb-16 lg:mx-auto lg:max-w-[1380px] lg:px-6 xl:px-0">
        <div className="grid grid-cols-1 items-start gap-8 md:gap-10 xl:grid-cols-[1fr_0.88fr] xl:gap-12">
          {/* Left */}
          <div className="pt-2 md:pt-6 xl:pt-10">
            <div className={`mb-6 inline-flex items-center rounded-full border border-border bg-muted px-4 py-1 font-medium text-muted-foreground ${isSpanish ? "text-[11px]" : "text-[12px]"}`}>
              {language === "es" ? "Confiado por más de 50,000 comerciantes de habilidades" : "Trusted by 50,000+ skill Traders"}
            </div>

            <h1 className={`mb-4 max-w-[620px] font-bold leading-[1.08] tracking-[-0.02em] text-foreground ${isSpanish ? "text-[32px] sm:text-[42px] md:text-[54px] xl:text-[70px]" : "text-[34px] sm:text-[46px] md:text-[58px] xl:text-[74px]"}`}>
              {language === "es" ? "Donde la habilidad es" : "Where Skill is"}
              <br />
              <span className="text-[#708f88]">{language === "es" ? "moneda" : "Currency"}</span>
            </h1>

            <p className={`mb-8 max-w-[520px] leading-[1.45] text-muted-foreground ${isSpanish ? "text-sm sm:text-base md:text-lg" : "text-base sm:text-lg md:text-xl"}`}>
              {language === "es"
            ...
            {/* Stats */}
            <div className="grid w-full max-w-[680px] grid-cols-3 overflow-hidden rounded-2xl border border-border bg-transparent px-1 py-3 sm:px-2 md:py-4">
          ...
          <div className="relative mx-auto h-80 w-full self-start sm:h-[420px] md:h-[520px] xl:h-[700px] xl:self-end">
            <div className="hero-image-clip absolute inset-0 overflow-hidden bg-card xl:-mt-16">
            ...
            {/* Bottom profile card */}
            <div className="absolute bottom-3 left-3 flex h-13 w-[calc(100%-88px)] max-w-[335px] items-center gap-2 rounded-full bg-card/95 px-2.5 py-2 shadow-[0_12px_32px_-20px_rgba(10,18,40,0.6)] backdrop-blur-md sm:bottom-5 sm:left-5 sm:h-[72px] sm:w-[335px] sm:gap-3 sm:px-3 xl:bottom-10">
              <Image
